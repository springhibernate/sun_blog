class UserInfosController < ApplicationController
  ...
  # GET /user_infos.xml
  def index
    @user_infos = UserInfo.paginate  :page=>params[:page],:order=>"id desc",:per_page=>PER_PAGE
  ...
  def create
    ...
    respond_to do |format|
      if @user_info.save
        format.html { redirect_to(user_infos_url, :notice => t('create_successfully')) }
  ...
  def update
    ...
    respond_to do |format|
      if @user_info.update_attributes(params[:user_info])
        format.html { redirect_to(user_infos_url, :notice => t('uodate_successfully')) }
        format.xml  { head :ok }
      else
